<script lang="ts">
    ...
    async function itemsToList(e: KeyboardEvent) {
        ...
        if (!dashboard) {
            ...
            if (!fetchresult.ok) {
                if (result.error.status == 429) {
                    inputerror = 'Enough items added for now. Try again soon or ask a student senate member for help!'
                } else {
        ...
        if (!dashboard) {
            ...
            await supabase
            .from('Items')
            .insert({ 'item_id': crypto.randomUUID(), 'text': input.trim(), 'checked': false, 'date': value.toString().split('T')[0], 'deleted': false, 'dashboard': true})
            .select()
            loading = false
            input=''
            value=today(getLocalTimeZone())
